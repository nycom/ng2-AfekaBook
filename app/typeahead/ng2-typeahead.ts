...
@Component({
    ...
    <div class="typeahead">
      ...
      <input #inputElement
        ...
        [(ngModel)]="input"
        type="text"
        [ngClass]="{'textbox': true, 'typeahead-input-has-selection': hasSelection()}"
        typeahead="off"
        spellcheck="false"
...
export class Typeahead implements OnInit {
    ...
    public populateSuggestions() {
        ...
        this.suggestions = this.list.filter(function (item) {
            //console.log(item[searchProperty].toLowerCase().indexOf(input.toLowerCase()) > -1);
    ...
    public populateTypeahead() {
        ...
        // Set the typeahead value
        if (this.input == "*")
            this.typeahead = this.input + (this.activeSuggestion[this.displayProperty] || '');
        else
